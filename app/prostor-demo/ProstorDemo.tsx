    ...
    <section className={styles.recordV7}>
      ...
      <form className={`${styles.recordLayout} ${styles.recordV7Form}`} onSubmit={submit}>
        ...
        <div className={styles.recordForm}>
          ...
          <label className={styles.range}><span>Jak silné to bylo?</span><div><small>slabé</small><input type="range" min="1" max="5" value={intensity} style={{ "--range-progress": `${(intensity - 1) / 4 * 100}%` } as CSSProperties} onChange={(event) => setIntensity(Number(event.target.value))} /><strong>{intensity}</strong><small>silné</small></div></label>
